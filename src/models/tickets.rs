...
pub async fn create(db_pool: Arc<PgPool>, new_ticket: TicketInput) -> Option<Uuid> {
    let res = sqlx::query!("INSERT INTO tickets (owner_id, concert_name, concert_date, barcode_data, price) VALUES ($1, $2, $3, $4, $5) returning id",
            new_ticket.owner_id,
...
pub async fn update(db_pool: Arc<PgPool>, id: Uuid, new_ticket: TicketInput) -> Option<Uuid> {
    let res = sqlx::query!("UPDATE tickets SET owner_id = $1, concert_name = $2, concert_date = $3, barcode_data = $4, price = $5, updated_at = $6 WHERE id = $7 returning id",
            new_ticket.owner_id,
            new_ticket.concert_name,
            new_ticket.concert_date,
            new_ticket.barcode_data,
